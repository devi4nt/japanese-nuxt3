...
<template>
  ...
  <table class="w-full text-white bg-black/70">
    <thead>
      <tr>
        <th class="text-left p-2 w-auto border">
          #
        </th>
        <th class="text-left p-2 w-1/4 border">
          Kanji
        </th>
        <th class="text-left p-2 w-1/4 border">
          Sino Japanese
    ...
    <tbody>
      ...
      >
        <td class="p-2 w-auto border">
          {{ number.decimal }}
        </td>
        <td class="p-2 w-1/4 border text-2xl">
          {{ number.sinoKanji.symbol }}
        </td>
        <td class="p-2 w-1/4 border">
          <InlinePhrase v-for="(phrase, index) in number.sinoPhrases" :key="index" :phrase="phrase" />
        </td>
        <td class="p-2 w-1/4 border">
          {{ number.nativeKanji?.symbol || '' }}
        </td>
        <td class="p-2 w-1/4 border">
