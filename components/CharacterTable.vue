...
const drop = (_event: MouseEvent, element: HTMLElement) => {
...
<template>
  <div>
    ...
    <div v-if="active" ref="mistakes" class="badge bg-custom-red w-8">
      {{ mistakeCount }}
    </div>
    <div v-if="active" ref="correct" class="badge bg-custom-green w-8">
      {{ correctCount }}
    </div>
    ...
    <table class="w-full text-white bg-black/70">
      <thead>
        <tr>
          ...
          <th v-if="table.rows" class="text-center w-10 p-2 border" />
        </tr>
      </thead>
      <tbody>
        <tr>
...
&nbsp;
    ...
    <div ref="dragSource" class="text-2xl" />
  </div>
</template>
